...
| `extensions/local.editor-interactions-2.2.2/` | Local extension for settings-driven click, word-selection, and dark-theme selection styling |

## Installation

1. Clone or download this repository.
2. Merge `settings-snippet.jsonc` into your VS Code user `settings.json`; do not replace the entire settings file.
3. Copy `extensions/local.editor-interactions-2.2.2` into `%USERPROFILE%\.vscode\extensions\`.
...
- `editor.selectionHighlight: true` keeps matching-word discovery in VS Code's native text model. CSS changes only the matching-word border/background, while the local extension does not scan the document.
