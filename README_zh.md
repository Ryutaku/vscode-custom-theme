...
材质层是面向 Windows 11 的可选功能。VS Code 目前没有把 Electron 的原生背景材质开放成设置项，因此脚本会在当前安装版本的 `out/main.js` 中加入一处带有明确标记的小修改：启用所选材质，同时移除 VS Code 自己的不透明启动背景色，让材质能够透过透明的工作台表面显示出来。脚本会在原文件旁创建安全备份；正常关闭材质时只删除本仓库加入的代码，不会整份恢复备份，因此不会覆盖其他修改。
