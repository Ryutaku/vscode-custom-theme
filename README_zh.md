...
材质层是面向 Windows 11 的可选功能。VS Code 目前没有把 Electron 的原生背景材质开放成设置项，因此脚本会在当前安装版本的 `out/main.js` 中加入两处带有明确标记的小修改：启用所选材质、移除 VS Code 自己的不透明启动背景色，并阻止主题服务在工作台加载完成后再次用实色盖住材质。脚本会在原文件旁创建安全备份；正常关闭材质时只删除本仓库加入的代码，不会整份恢复备份，因此不会覆盖其他修改。
